...
## Discovery Round 63
- [ ] D63.1: `apps/mobile/store/dictionariesSlice.ts` `updatePair` defensive cleanup queries database with plain paths but DB stores URIs — lines 159-168 use `getDictionaryFilePath(nativeLang, studiedLang, newVersion)` (plain paths) in `SELECT 1 FROM installed_dictionaries WHERE file_path = ?`, but the database stores URIs from `result.first.filePath`/`result.second.filePath`; query never matches so cleanup always deletes target files even when valid DB rows exist, causing potential data loss on update retry [--P-B-]

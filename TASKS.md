...
- [ ] T24.4: Add tests for `dictionaryVersions.ts` (`getPairDownloadSize`), the `installed_dictionaries` schema migration (old two-column PK → new PK with backfilled `version`), and `runDictionaryMaintenance()`'s three cleanup cases [--P-B-!]

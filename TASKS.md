...
- [ ] T17.2: Update `apps/mobile/app/onboarding.tsx` to use `startPairDownload` — replace the single-dict download with the pair download helper; call `addDictionary` twice on success (once per direction); show a single combined progress bar across both downloads [QRP-B-!]

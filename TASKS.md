...
## Discovery Round 62
- [ ] D62.1: `apps/mobile/app/(tabs)/settings.tsx` does not handle the `updatePair` URL parameter from `DirectionSelector` navigation — when a user taps Update in the direction picker, it navigates to Settings with `?updatePair={pairKey}` but settings.tsx never reads this parameter to automatically open the DownloadConfirmationDialog for that pair

...
- [ ] T21.4: Update `runMigrations()` in `apps/mobile/db/index.ts` — the PK change can't be done with `ALTER TABLE ADD COLUMN` like the existing `settings.lookup_direction` migration, so detect the old two-column-PK table (e.g. via `PRAGMA table_info`) and, if found, recreate `installed_dictionaries` with the new schema, copying existing rows with `version` backfilled to `'2_2025-11'` (the only version ever downloaded before this feature existed); leave existing `file_path` values untouched — no file renaming

...
## Discovery Round 4
- [ ] D4.1: Fix array index anti-pattern in ResultsList.tsx senseList rendering - The senseList.map uses key={i} which can cause reconciliation issues when the list order changes. Replace with key={sense} or key={\`${item.writtenRep}-${i}\`} to ensure stable keys.

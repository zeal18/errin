...
- [ ] T21.5: Update `apps/mobile/lib/dictionaryDownload.ts` — `getDictionaryFileName`/`getDictionaryFilePath`/`getDictionaryUrl` take a `version` parameter and default to `CURRENT_DICTIONARY_VERSION.id` from the new registry instead of the local `WIKDICT_VERSION` constant (which is removed); filenames become `{sourceLang}-{targetLang}-{version}.sqlite3`

...
export function AddSourceLanguageModal({
  visible,
  onClose,
}: {
  visible: boolean;
  ...
  // Get unique installed target languages
  ...
  // Available languages are those not already installed as source
  const availableLanguages = SUPPORTED_LANGUAGES.filter(
    (l) => !installedSourceLangs.includes(l.code)
  );
  ...
  const startDownloads = async () => {
    ...
    setStep('download');
    const items: DownloadItem[] = installedTargetLangs.map((targetLang) => ({
    ...
    for (const targetLang of installedTargetLangs) {
  ...
  const onRetry = (targetLang: string) => {
    if (!selectedLang) return;
          ...
          >
            ...
            <View className="p-4">
              <Text className="text-sm text-neutral-600 mb-4">
                Pick a language to use as a source. Dictionaries will be downloaded for all
                installed target languages.
              </Text>
        ...
        >
          ...
          <View className="p-4">
            <Text className="text-sm text-neutral-600 mb-4">
              ...
              } for {getLanguageName(selectedLang ?? '') ?? selectedLang} → all installed targets
            </Text>
                ...
                return (
                  ...
                  >
                    <Text className="text-sm font-medium text-neutral-900">
                      {getLanguageName(item.sourceLang) ?? item.sourceLang} → {targetName}
                    </Text>
                    {item.status === 'pending' && (
                      <View className="mt-2">
                    ...
                    {item.status === 'error' && (
                      <View className="mt-2">
                        ...
                        <Pressable
                          className="mt-1"
                          onPress={() => onRetry(item.targetLang)}
                        >
